...
{ python_version_code || true ; } > "${TMP}/code.txt" 2>/dev/null
...
{ alpine_venv_code || true ; } > "${TMP}/code.txt" 2>/dev/null
...
{ venv_with_packages_code || true ; } > "${TMP}/code.txt" 2>/dev/null
...
{ debian_venv_code || true ; } > "${TMP}/code.txt" 2>/dev/null
...
{ pip_tools_code || true ; } > "${TMP}/code.txt" 2>/dev/null
