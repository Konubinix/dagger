...
* Reproducibility helpers
  ...
  test command files.  Each =dagger call= bash block tangles to
  =tests/commands/<name>=.  =run.sh= executes the bash blocks and
  populates =#+RESULTS:=.  Tangling then writes those results to
  =tests/expected/= via noweb references.  The test runner simply
  iterates over command files, runs each one, and compares stdout
...
* Org-mode pin
  ...
  The scripts =tangle-nodagger.sh= and =run.sh= clone a pinned
  org-mode commit. The pin lives in a single file so it cannot
  diverge. The bash scripts inline the hash at tangle time via noweb.
...
* Tangle without dagger (bootstrap)
  ...
  This script solves the chicken-and-egg problem: =tangle.sh= is
  itself a tangled file, so you need this bootstrap script to
  generate it the first time. It uses host tools (emacs, git, ruff).
...
* Tangle script
  ...
  The daily-use tangle script tangles all org files using host emacs.
  ...
  #+begin_src bash :tangle tangle.sh :shebang "#!/usr/bin/env bash" :noweb yes
...
* Run script
  ...
  Executes org-babel blocks using emacs in batch mode and saves the
  results. Populates the =#+RESULTS:= blocks that tangling writes
  to =tests/expected/= via noweb references.

  #+begin_src bash :tangle run.sh :shebang "#!/usr/bin/env bash" :noweb yes
